...
def build_schema_report(
    *,
    ...
    for row in monthly_summary or []:
        ...
        bank_name = str(df.get("bank", pd.Series(["Unknown Bank"])) .dropna().astype(str).head(1).iloc[0]) if not df.empty else "Unknown Bank"
    ...
    high_value = []
    if not df.empty and "credit" in df.columns:
        for _, row in df[df["credit"].apply(safe_float) >= 100000].iterrows():
            high_value.append(
                {
    ...
    if not df.empty:
        for _, row in df.iterrows():
            classified = bool(str(row.get("rule_category_code") or "").strip())
            if classified:
                continue
            amount = safe_float(row.get("credit")) if safe_float(row.get("credit")) > 0 else safe_float(row.get("debit"))
            tx_type = "CREDIT" if safe_float(row.get("credit")) > 0 else "DEBIT"
            unclassified.append(
                {
                    "date": _as_date(row.get("date")),
                    "description": str(row.get("description") or ""),
    ...
    report = {
        ...
        "consolidated": {
            ...
            "total_own_party_cr": 0.0,
            "total_own_party_dr": 0.0,
            "total_related_party_cr": 0.0,
            "total_related_party_dr": 0.0,
            "total_reversal_cr": 0.0,
            "total_returned_cheques_inward": 0.0,
            "total_returned_cheques_outward": 0.0,
            "total_loan_disbursement_cr": 0.0,
            "total_fd_interest_cr": 0.0,
            "total_round_figure_cr": 0.0,
            "total_high_value_cr": round(float(sum(x["amount"] for x in high_value)), 2),
            "total_cash_deposits": 0.0,
            "total_cash_withdrawals": 0.0,
            "total_cheque_deposits": 0.0,
            "total_cheque_issues": 0.0,
            "total_loan_repayment_dr": 0.0,
            "total_salary_paid": 0.0,
            "total_statutory_epf": 0.0,
            "total_statutory_socso": 0.0,
            "total_statutory_tax": 0.0,
            "total_statutory_hrdf": 0.0,
        ...
        "own_related_transactions": {
            "summary": {
                ...
                "related_party_cr": 0.0,
                "related_party_dr": 0.0,
                "own_party_cr_pct": 0.0,
                "own_party_dr_pct": 0.0,
                "related_party_cr_pct": 0.0,
                "related_party_dr_pct": 0.0,
            },
            "transactions": [],
        },
